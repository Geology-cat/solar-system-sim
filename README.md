...
**方法 0: DMG 同梱のスクリプトを使う（配布版）**

1. `SolarSystemSim` を右どなりの `Applications` フォルダへドラッグしてインストールする
2. `Gatekeeper解除.scpt` をダブルクリックする（スクリプトエディタが開く）
3. 上部の「実行」（▶）ボタンを押す

隔離属性を取り除くだけで、システムの設定は変更せず、他のアプリの扱いにも影響しない。
解除後はそのままアプリを起動するか聞いてくる。
